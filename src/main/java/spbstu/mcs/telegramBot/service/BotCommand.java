...
public class BotCommand {
        ...
        return cryptoInformation.showCurrentPrice()
            .map(jsonPrice -> {
                try {
                    ...
                    return String.format("📊 Текущий курс криптовалюты\n" +
                                       "➡️ %s (%s):\n" +
                                       "💰 %s (по курсу %s)\n" +
            ...
            .onErrorResume(throwable -> Mono.just("❌ Ошибка при получении текущей цены: " + throwable.getMessage()));
    }
    ...
    public Mono<String> handlerSetAlertPerc(String args, String chatId) {
        ...
        try {
            // Проверяем формат процентов (до 2 знаков после запятой)
            if (!processedArgs[1].matches("^\\d+(\\.\\d{1,2})?$") || !processedArgs[2].matches("^\\d+(\\.\\d{1,2})?$")) {
                return Mono.just("❌ Неверный формат процента! Используйте числа с максимум 2 знаками после запятой, например: 5.25 3.75");
            }
            ...
            return Mono.just("❌ Неверный формат процента! Используйте числа, например: 5.25 3.75");
        }
    }
            ...
            .flatMap(notifications -> {
                ...
                for (Notification notification : notifications) {
                    ...
                    if (type.equals("VALUE") || type.equals("PERCENT")) {
                        ...
                        // Для EMA алертов всегда показываем время работы
                        message.append(String.format("   Время работы: %s\n", 
                            formatDuration(notification.getStartTimestamp())));
                    }
    ...
    private Mono<Void> processCommandInternal(String command, String[] args, String chatId) {
        ...
        return switch (normalizedCommand) {
            ...
            case "/show_current_price" -> handlerShowCurrentPrice(argsStr).then();
            case "/show_price_history" -> handlerShowPriceHistory(argsStr).then();
            case "/compare_currency" -> handlerCompareCurrency(argsStr).then();
            case "/portfolio" -> handlerPortfolio(argsStr, chatId).then();
            case "/get_portfolio_price" -> handlerGetPortfolioPrice(argsStr, chatId).then();
            case "/get_assets_price" -> handlerGetPortfolioAssets(argsStr, chatId).then();
            case "/balance" -> handlerBalance(chatId).then();
            case "/set_alert_val" -> handlerSetAlertVal(argsStr, chatId).then();
            case "/set_alert_perc" -> handlerSetAlertPerc(argsStr, chatId).then();
            case "/set_alert_ema" -> handlerSetAlertEMA(argsStr, chatId).then();
            case "/my_alerts" -> handlerMyAlerts(argsStr, chatId).then();
            case "/delete_alert" -> handlerDeleteAlert(argsStr, chatId).then();
            case "/delete_all_alerts" -> handlerDeleteAllAlerts(argsStr, chatId).then();
            case "/add" -> handlerAdd(args, chatId).then();
            case "/remove" -> handlerRemove(args, chatId).then();
            case "/delete_asset" -> handlerDeleteAsset(argsStr, chatId).then();
            case "/delete_all_assets" -> handlerDeleteAllAssets(argsStr, chatId).then();
            case "/authors" -> handlerAuthors(argsStr).then();
    ...
    public Mono<String> handlerAdd(String[] args, String chatId) {
        ...
        try {
            ...
            if (portfolios.stream()
                ...
                Portfolio newPortfolio = portfolioService.createPortfolio(user.getUserTgName(), Currency.Fiat.USD, chatId);
            ...
            return Mono.zip(
                ...
                try {
                    ...
                    return Mono.just(String.format("✅ Добавлено %.2f %s\n" +
                            "💰 Текущая цена: %.2f %s\n" +
                            "💵 Стоимость: %.2f %s\n\n" +
                            "📊 Всего в портфеле: %.2f %s\n" +
                            "💎 Общая стоимость актива: %.2f %s",
                            amount.setScale(2, RoundingMode.HALF_UP), crypto.getCode(),
                            currentPrice, Currency.Fiat.getCurrentFiat().getCode(),
                            totalValue, Currency.Fiat.getCurrentFiat().getCode(),
                            totalAmount.setScale(2, RoundingMode.HALF_UP), crypto.getCode(),
        ...
        return Mono.zip(
            ...
            try {
                ...
                return Mono.just(String.format("💰 Баланс портфеля:\n\n" +
                        ...
                        portfolio.getCount().setScale(2, RoundingMode.HALF_UP),
                        currentPrice, Currency.Fiat.getCurrentFiat().getCode(),
                        totalValue, Currency.Fiat.getCurrentFiat().getCode()));
            } catch (Exception e) {
                log.error("Ошибка при обработке данных о цене", e);
    ...
    public Mono<String> handlerRemove(String[] args, String chatId) {
        ...
        try {
            ...
            if (portfolio.getCount().compareTo(amount) < 0) {
                return Mono.just(String.format("❌ Недостаточно средств. В портфеле: %.2f %s", 
                    portfolio.getCount().setScale(2, RoundingMode.HALF_UP), crypto.getCode()));
            }
            ...
            return Mono.zip(
                ...
                try {
                    ...
                    return Mono.just(String.format("✅ Удалено %.2f %s\n\n" +
                            "📊 Всего в портфеле: %.2f %s\n" +
                            "💎 Общая стоимость актива: %.2f %s",
                            amount.setScale(2, RoundingMode.HALF_UP), crypto.getCode(),
                            totalAmount.setScale(2, RoundingMode.HALF_UP), crypto.getCode(),
