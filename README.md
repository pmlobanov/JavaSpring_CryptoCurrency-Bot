...
| №  | Команда | Описание |
|----|---------|----------|
| 11 | `/my_alerts` | Показать активные оповещения |
| 12 | `/delete_alert_<тип> <валюта>` | Удалить активное оповещение по типу и валюте |
| 13 | `/delete_all_alerts` | Удаление всех оповещений |
| 14 | `/portfolio` | Просмотр портфеля |
| 15 | `/get_portfolio_price` | Отслеживание цены портфеля за указанный период |
| 16 | `/get_assets_price` | Показать убыток/прибыль по каждой криптовалюте в портфеле |
...
- `Service` (Сервис):
  ...
  - `CryptoPortfolioManager`: Управление портфелями пользователей, включая добавление/удаление криптовалют, расчет стоимости и отслеживание изменений цен.
